...
// Return the x,y coordinate points of all glyphs in the text.
// On-curve points will be drawn in blue, off-curve points will be drawn in red.
//
// ctx - A 2D drawing context, like Canvas.
// text - The text to create.
...
// Options is an optional object that contains:
// - kerning - Whether to take kerning information into account. (default: true)
Font.prototype.getPoints = function (ctx, text, x, y, fontSize, options) {
    var allPoints = [];
    this.forEachGlyph(text, x, y, fontSize, options, function (glyph, x, y, fontSize) {
        allPoints.push(glyph.getPoints(ctx, x, y, fontSize));
    });
    return allPoints;
};
...
//Returns the x,y coordinates of the all the red and blue points. 
Glyph.prototype.getPoints = function (ctx, x, y, fontSize) {

    function computePoints(l, x, y, scale) {
        var j, PI_SQ = Math.PI * 2;
        for (j = 0; j < l.length; j += 1) {
            l[j].x = x + (l[j].x * scale);
            l[j].y = y + (l[j].y * scale);
    ...
    var scale, i, path, cmd, allCircles;
    x = x !== undefined ? x : 0;
    y = y !== undefined ? y : 0;
    fontSize = fontSize !== undefined ? fontSize : 24;
    scale = 1 / this.font.unitsPerEm * fontSize;
    ...
    for (i = 0; i < path.commands.length; i += 1) {
        cmd = path.commands[i];
        if (cmd.x !== undefined) {
            allCircles.push({x: cmd.x, y: -cmd.y});
        }
        if (cmd.x1 !== undefined) {
            allCircles.push({x: cmd.x1, y: -cmd.y1});
        }
        if (cmd.x2 !== undefined) {
            allCircles.push({x: cmd.x2, y: -cmd.y2});
        }
    }
    computePoints(allCircles, x, y, scale);
    return allCircles;
};
